import { GetPostsRequestType, GetPostsResponseType, TagType } from "../model/types";

const API_BASE_URL = "/api/posts";

export const postApi = {
  // 게시물 목록 조회
  getPosts: async (params: GetPostsRequestType): Promise<GetPostsResponseType> => {
    const response = await fetch(`${API_BASE_URL}?limit=${params.limit}&skip=${params.skip}`);
  ...
  getPostsByTag: async (tag: string): Promise<GetPostsResponseType> => {
    const response = await fetch(`${API_BASE_URL}/tag/${encodeURIComponent(tag)}`);
